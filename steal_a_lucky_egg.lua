...
local RarityWeights = {
    ...
    -- Specific Brainrot & Lucky Characters / Tiers
    ["skibidi titan"] = 9500,
    ["gigachad"] = 8500,
    ["mewing"] = 8000,
    ["sigma"] = 7500,
...
local function GetEggRarityScore(obj, prompt)
    local score = 100
    local detectedName = obj and obj.Name or "Lucky Egg"
    ...
    if obj and obj:IsA("Instance") then
        pcall(function()
            for attrName, attrVal in pairs(obj:GetAttributes()) do
                ...
                if type(attrVal) == "number" and (attrName:lower():find("price") or attrName:lower():find("value") or attrName:lower():find("tier") or attrName:lower():find("rarity") or attrName:lower():find("level") or attrName:lower():find("luck")) then
...
local function FindRarestEgg(hrpPosition)
    ...
    local bestEggName = "Rare Lucky Egg"
            ...
            if targetPos then
                ...
                if not isCoolingDown then
                    ...
                    if distFromBase > 12 then
                        ...
                        local isEggOrSteal = act:find("steal") or act:find("take") or act:find("grab") or act:find("egg") or act:find("lucky") or act:find("brainrot") or act:find("pick") or act:find("collect") or pName:find("egg") or pName:find("lucky") or pName:find("brainrot") or act == "" or act == " "
...
-- 8. Auto Sell (Pets / Unwanted Eggs / Items)
AddToggleRow("Auto Sell", "AutoSell", function(state) end)

-- 9. Auto Hatch Eggs (Hatch incubated eggs placed in Base)
AddToggleRow("Auto Hatch Eggs", "AutoHatch", function(state) end)

-- 10. Auto Upgrade Base & Treadmills
...
-- 11. Rare Lucky Egg ESP (Neon Magenta Glowing Highlight + Billboard)
...
-- 12. Lucky Egg ESP (Gold Highlight on All Eggs)
...
-- 13. Player ESP & Base Defense Radar
...
-- 17. Integrated WalkSpeed Row with - / + Pill Adjuster
...
-- ====================================================
-- 3. AUTO SELL ENGINE (SELLS PETS / UNWANTED EGGS)
-- ====================================================
task.spawn(function()
    while true do
        task.wait(0.5)
...
-- ====================================================
-- 4. AUTO HATCH EGGS (HATCHES EGGS IN BASE SLOTS/NESTS)
-- ====================================================
task.spawn(function()
    while true do
        task.wait(0.3)
...
-- ====================================================
-- 5. AUTO UPGRADE BASE & TREADMILLS ENGINE
-- ====================================================
task.spawn(function()
    while true do
        task.wait(0.8)
...
-- ====================================================
-- 6. RARE LUCKY EGG ESP ENGINE (MAGENTA NEON HIGHLIGHT & LIVE TAG)
-- ====================================================
...
-- ====================================================
-- 7. LUCKY EGG ESP ENGINE (GOLD HIGHLIGHT)
-- ====================================================
RunService.RenderStepped:Connect(function()
