...
-- 5 Clean Feature States (Auto Rebirth 100% Removed)
local Toggles = {
    InfiniteWins = false,
    AutoTrain = false,
    Fly = false,
...
--------------------------------------------------------------------
-- 5. BULLETPROOF AUTO TRAIN ENGINE (Hyper-Speed +1 Speed Booster)
--------------------------------------------------------------------
task.spawn(function()
    while true do
        task.wait(0.1)
        if Toggles.AutoTrain then
            pcall(function()
                ...
                -- Layer 1: Emulate rapid walking / running state
                hum:ChangeState(Enum.HumanoidStateType.Running)

                -- Layer 2: Auto Equip & Activate Ninja Tools
                ...
                -- Layer 3: Touch Treadmills & Training Pads
                for _, obj in ipairs(Workspace:GetDescendants()) do
                ...
                -- Layer 4: ReplicatedStorage Speed & Step Remotes
                for _, remote in ipairs(ReplicatedStorage:GetDescendants()) do
                    if not Toggles.AutoTrain then break end
                    if remote:IsA("RemoteEvent") then
                        local rn = remote.Name:lower()
...
--------------------------------------------------------------------
-- 6. EXACT JUNEJO ULTRA SCRIPT HUB CLASSIC UI (5 CLEAN FEATURES)
--------------------------------------------------------------------
...
-- 5 Clean Toggle Rows ONLY (NO AUTO REBIRTH)
