--[[
    JUNEJO ULTRA SCRIPT HUB - +1 MONKEY EVOLUTION
    Target Game: +1 Monkey Evolution (Roblox)
    Game Link: https://www.roblox.com/games/91701030914075/1-Monkey-Evolution
...
pcall(function()
    local names = {"JunejoMonkeyEvolutionUI", "JunejoMonkeyUI", "JunejoHubUI"}
...
TitleLabel.Text = "+1 MONKEY EVOLUTION"
...
pcall(function()
    StarterGui:SetCore("SendNotification", {
        ...
        Text = "+1 Monkey Evolution Loaded!",
        Duration = 4
    })
end)
...
--------------------------------------------------------------------
-- 4. AUTO CLICK / TRAIN (+1 Power / Banana - SAFE DIRECT ENGINE)
--------------------------------------------------------------------
task.spawn(function()
    while true do
        task.wait(0.04)
        if Toggles.AutoClick then
            pcall(function()
                ...
                -- Auto activate equipped tools safely
                if char then
                ...
                -- Direct Power / Click / Banana Remote Sweeper
                for _, obj in ipairs(ReplicatedStorage:GetDescendants()) do
                    if not Toggles.AutoClick then break end
                    local nameLower = obj.Name:lower()
                    if nameLower:find("click") or nameLower:find("train") or nameLower:find("power") or 
                       nameLower:find("strength") or nameLower:find("banana") or nameLower:find("tap") or 
                       nameLower:find("punch") or nameLower:find("gain") or nameLower:find("monkey") then
...
local function GetRebirthRequirementsInfo()
    ...
    pcall(function()
        ...
        local powerVal = leaderstats and (leaderstats:FindFirstChild("Power") or leaderstats:FindFirstChild("Strength") or leaderstats:FindFirstChild("Bananas") or leaderstats:FindFirstChild("Banana"))
        ...
        local curPower = powerVal and powerVal.Value or 0
        local curWins = winsVal and winsVal.Value or 0
        local curRebirth = rebirthVal and rebirthVal.Value or 0

        info = "Rebirth: " .. tostring(curRebirth) .. " | Power: " .. tostring(curPower) .. (winsVal and (" | Wins: " .. tostring(curWins)) or "")
    end)
    return info
end
            ...
            pcall(function()
                ...
                if LocalPlayer:FindFirstChild("PlayerGui") then
                    for _, gui in ipairs(LocalPlayer.PlayerGui:GetChildren()) do
                        if gui:IsA("ScreenGui") and gui.Name ~= "JunejoMonkeyEvolutionUI" then
                            for _, btn in ipairs(gui:GetDescendants()) do
                                if btn:IsA("TextButton") or btn:IsA("ImageButton") then
                                    ...
                                    if (bName:find("rebirth") or bText:find("rebirth") or parentName:find("rebirth") or bName:find("evolve") or bText:find("evolve")) and 
                ...
                for _, obj in ipairs(ReplicatedStorage:GetDescendants()) do
                    ...
                    if nameLower:find("rebirth") or nameLower:find("prestige") or nameLower:find("evolve") or 
                       nameLower:find("dorebirth") or nameLower:find("buyrebirth") or nameLower:find("monkeyrebirth") then
                        if obj:IsA("RemoteEvent") then
                            pcall(function()
                                obj:FireServer()
                                obj:FireServer(1)
