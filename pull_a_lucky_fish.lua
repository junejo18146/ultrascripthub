...
for _, name in ipairs({"JunejoLuckyFishUI", "JunejoHubUI_LuckyFish"}) do
...
--------------------------------------------------------------------
-- HELPER FUNCTIONS & REMOTES SCANNER
--------------------------------------------------------------------
local function FireMatchingRemotes(keywords, args)
    pcall(function()
...
local function TriggerPrompts(keywords)
    pcall(function()
        for _, prompt in ipairs(Workspace:GetDescendants()) do
            if prompt:IsA("ProximityPrompt") then
                local name = (prompt.Parent and prompt.Parent.Name or prompt.Name):lower()
                for _, kw in ipairs(keywords) do
...
-- Find Last Zone & Base Positions
local function GetZonePositions()
    local lastZonePart = nil
    local basePart = nil

    pcall(function()
        local maxDist = 0
        local spawnPart = Workspace:FindFirstChild("SpawnLocation") or (LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart"))
        local origin = spawnPart and spawnPart.Position or Vector3.new(0, 0, 0)
        ...
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("BasePart") or obj:IsA("Model") then
                ...
                if part then
                    if n:find("base") or n:find("plot") or n:find("dock") or n:find("home") then
                        if not basePart then basePart = part end
                    end
                    if n:find("zone") or n:find("ocean") or n:find("deep") or n:find("water") or n:find("island") or n:find("lake") then
    ...
    while true do
        ...
        if Toggles.AutoFish then
            pcall(function()
                ...
                if backpack and hum then
                    for _, tool in ipairs(backpack:GetChildren()) do
                        if tool:IsA("Tool") then
                            ...
                            if tname:find("rod") or tname:find("fish") or tname:find("pole") or tname:find("pull") then
                                hum:EquipTool(tool)
                                break
                            end
                        end
                ...
                -- Fire Fishing & Pull Remotes
                FireMatchingRemotes({"cast", "fish", "reel", "pull", "catch", "hook", "bobber", "bite", "strike"}, {"Cast", true, 1})
                FireMatchingRemotes({"pullfish", "fastpull", "instantpull", "reelfish", "claimfish"}, {true, 1})
                TriggerPrompts({"fish", "cast", "reel", "pull", "catch"})
            end)
        end
    end
end)

-- 4. Instant Last Zone (Fast catch in last zone & fast return)
task.spawn(function()
    while true do
        task.wait(0.5)
        if Toggles.InstantLastZone then
            pcall(function()
                ...
                if hrp then
                    local lastZone, baseSpot = GetZonePositions()
                    if lastZone then
                        -- Teleport to deepest / last zone for highest rarity fish
                        hrp.CFrame = lastZone.CFrame + Vector3.new(0, 3, 0)
                        task.wait(0.2)
                        FireMatchingRemotes({"cast", "fish", "pull", "reel", "catch"}, {})
                    end
                end
            end)
        end
    ...
    while true do
        ...
        if Toggles.AutoCollectCash then
            pcall(function()
                ...
                -- Fire Collector Remotes
                FireMatchingRemotes({"collect", "claim", "income", "cash", "money", "tank", "payout", "deposit", "withdraw"}, {})
                TriggerPrompts({"collect", "claim", "cash", "money", "tank", "coin"})

                -- Touch Collector Pads & Floating Money
                if hrp then
                    for _, obj in ipairs(Workspace:GetDescendants()) do
                        if not Toggles.AutoCollectCash then break end
                        if obj:IsA("BasePart") then
                            local n = obj.Name:lower()
                            if n:find("collector") or n:find("cash") or n:find("coin") or n:find("money") or n:find("deposit") or n:find("pad") then
                                if firetouchinterest then
                                    firetouchinterest(hrp, obj, 0)
                                    task.wait(0.005)
                                    firetouchinterest(hrp, obj, 1)
                                end
                                if n:find("coin") or n:find("cash") or n:find("drop") then
...
-- 6. Auto Sell Fish (Low Tier Priority)
task.spawn(function()
    while true do
        task.wait(1.0)
        if Toggles.AutoSellFish then
            pcall(function()
                -- Remotes for selling low tier fish
                FireMatchingRemotes({"sell", "sellfish", "selllow", "sellcommon", "sellduplicate", "sellall"}, {"Common", "Uncommon", true})
                TriggerPrompts({"sell", "merchant", "shop", "trader"})
                ...
                if hrp then
                    for _, obj in ipairs(Workspace:GetDescendants()) do
                        ...
                        if obj:IsA("BasePart") and (obj.Name:lower():find("sell") or obj.Name:lower():find("merchant")) then
                            if firetouchinterest then
                                firetouchinterest(hrp, obj, 0)
                                task.wait(0.01)
                                firetouchinterest(hrp, obj, 1)
...
task.spawn(function()
    while true do
        ...
        if Toggles.AutoRebirth then
            pcall(function()
                FireMatchingRemotes({"rebirth", "prestige", "ascend", "rankup"}, {"Rebirth", true})
            end)
        end
    end
end)
